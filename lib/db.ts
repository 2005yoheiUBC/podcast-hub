...
export async function initDb() {
  ...
  // Per-user tables
  await sql`CREATE TABLE IF NOT EXISTS user_category_weights (user_id TEXT NOT NULL, category TEXT NOT NULL, weight REAL NOT NULL DEFAULT 1.0, updated_at BIGINT NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())::bigint, PRIMARY KEY (user_id, category))`
  await sql`ALTER TABLE interactions ADD COLUMN IF NOT EXISTS user_id TEXT`
  await sql`ALTER TABLE episodes ADD COLUMN IF NOT EXISTS user_id TEXT`
}
